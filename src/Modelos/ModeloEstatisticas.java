 ...
 */
public class ModeloEstatisticas {
     ...
     */
    public ModeloEstatisticas(String nomeJogadorOuEquipe, String nomeTorneio, int partidasJogadas, int vitorias, int derrotas) {
    ...
    private List<ModeloEstatisticas> estatisticasList;

    /**
     * Construtor da classe. Inicializa a lista de estatísticas.
     */
    public ModeloEstatisticas() {
    ...
    public void atualizarRanking() {
        ...
        Map<String, List<ModeloEstatisticas>> estatisticasPorTorneio = estatisticasList.stream()
                .collect(Collectors.groupingBy(ModeloEstatisticas::getNomeTorneio));

        // Para cada torneio, ordena os participantes e atribui os rankings
        for (List<ModeloEstatisticas> statsDoTorneio : estatisticasPorTorneio.values()) {
            // Ordena por pontos em ordem decrescente
            statsDoTorneio.sort(Comparator.comparingInt(ModeloEstatisticas::getPontos).reversed());
     ...
     */
    public List<ModeloEstatisticas> filtrarPorTorneio(String nomeTorneio) {
    ...
    public void registrarVitoria(String nomeJogadorOuEquipe, String torneioId) {
        ModeloEstatisticas estatistica = buscarOuCriarEstatistica(nomeJogadorOuEquipe, torneioId);
    ...
    public void registrarDerrota(String nomeJogadorOuEquipe, String nomeTorneio) {
        ModeloEstatisticas estatistica = buscarOuCriarEstatistica(nomeJogadorOuEquipe, nomeTorneio);
     ...
     */
    private ModeloEstatisticas buscarOuCriarEstatistica(String nomeJogadorOuEquipe, String nomeTorneio) {
        for (ModeloEstatisticas estatistica : estatisticasList) {
        ...
        ModeloEstatisticas novaEstatistica = new ModeloEstatisticas(
    ...
    public String exibirRanking() {
        ...
        for (String nomeTorneio : nomesTorneiosOrdenados) {
            ...
            List<ModeloEstatisticas> statsDoTorneio = estatisticasPorTorneio.get(nomeTorneio);

            // Ordena pela posição no ranking (que foi definida em atualizarRanking)
            // ou por pontos, se preferir recalcular a ordem aqui.
            statsDoTorneio.sort(Comparator.comparingInt(ModeloEstatisticas::getRanking));

            if (statsDoTorneio.isEmpty()) {
                sb.append("  Nenhum participante neste torneio.\n");
            } else {
                for (ModeloEstatisticas estatistica : statsDoTorneio) {
     ...
     */
    public List<ModeloEstatisticas> getEstatisticasList() {
